...
class DenoisingNet_MLP_3(torch.nn.Module):
    def __init__(
        ...
        self.unfold = torch.nn.Unfold(kernel_size=(self.patch_size, self.patch_size),stride=2)
        self.unfold_1 = torch.nn.Unfold(kernel_size=(self.patch_size, self.patch_size), stride=8)
        ...
        #
        #
        self.Z_features = torch.nn.Sequential(
            torch.nn.Conv2d(512, 512, kernel_size=3, stride=2, padding=0, groups=512),
            torch.nn.ReLU(),
        ...
        self.Z_max = torch.nn.AdaptiveMaxPool2d((1, 1)).to(device)
        self.Z_linear = torch.nn.Linear(512, 512).to(device)

        self.classifier = torch.nn.Sequential(
            torch.nn.Linear(512*5*5, 256*5*5),
            torch.nn.ReLU(),
            torch.nn.Linear(256*5*5, 1),
    ...
    def forward(self, x):
        ...
        unfold_extract = unfold_extract[:, :self.indice, :]
        unfold_extract = unfold_extract.contiguous()  # (N,256,64)
        unfold_extract = unfold_extract.to(self.device)  # (N,256,64)
        ...
        lin = self.pd1(unfold.to(self.device)).clamp(min=0)
        ...
        z1 = self.soft_thresh(y, l)
        for t in range(self.T):
            z1 = self.soft_thresh(torch.matmul(z1, S) + (1 / self.c) * y, l)
        ...
        x_pred = torch.matmul(z1, Dict_concat.transpose(-1, -2))
        x_pred = torch.clamp(x_pred, min=self.min_v, max=self.max_v)
        x_pred = W * x_pred
        x_pred = x_pred.transpose(1, 2)
        ...
        fold = torch.nn.functional.fold(
            x_pred, output_size=(w, h), kernel_size=(self.patch_size, self.patch_size),stride=2
        )

        norm = torch.nn.functional.fold(
            normalize,
            output_size=(w, h),
            kernel_size=(self.patch_size, self.patch_size),stride=2
        )
        ...
        z_clf = z
        z_clf = self.change_shape(z_clf, 105, 105).contiguous()
        z_clf_out = self.Z_features(z_clf)
        # print(z_clf_out.shape)
        # print(z_clf_out.shape)
#---------------使用senet进行原子之间权重的计算-----------
        z_se = self.Z_max(z_clf)
        z_se = z_se.view(z_se.size(0), -1)
        z_se = self.Z_linear(z_se)
        z_se = torch.sigmoid(z_se)
        z_se = z_se.view(z_se.size(0), z_se.size(1), 1, 1)
        z_label_out = z_clf_out *  z_se
#-------------------------------------------------------
        z_label_out = z_label_out.view(z_label_out.size(0), -1).contiguous()
        z_label_out = self.classifier(z_label_out)
